...
3. Turn off Launch GPClient at login, if enabled.
4. Quit GPClient and move the application to Trash.
...
The helper uses `/Library/Application Support/GPClient` for private session bundles and journals.
Empty staging directories may remain after normal use; they contain no credentials.

Connection preferences remain in the current user's `com.beelte.gpclient` preferences domain.
Removing the application does not erase the saved address or browser choice.

If you explicitly made GPClient the handler for `globalprotectcallback:`, restore your preferred VPN application's handler when removing GPClient.
