...
| Reference portal | The wrapper uses `vpn.example.com`. | Treat Reference provider as a validation example. Replace the fixed address with user configuration. |
...
Use Reference provider as the first real compatibility check, not as a product restriction.
...
Do not prefill Reference provider or provide a company-specific preset.
...
- The configured address drives every connection stage; the application contains no fixed company portal or Reference provider-specific behavior.
