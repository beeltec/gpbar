...
| Initial portal | The wrapper uses `vpn.example.com`. | Offer an Reference provider preset during setup. Keep the portal editable. |
...
- Reference provider preset, with editable display name and portal hostname.
...
Provide an accessibility label such as “GPClient, connected to Reference provider VPN”.
...
- The real Reference provider connection works without Terminal or developer tools.
